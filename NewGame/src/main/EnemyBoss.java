...
public class EnemyBoss extends GameObject {
	...
	public void tick() {
		...
		if (timer2 <= 0) {
			...
			if (spawn == 0) {
				EnemyBossBullet enemyBossBullet = new EnemyBossBullet((int) x + 70, (int) y + 70, ID.EnemyBossBullet,
						handler);
				handler.addObject(enemyBossBullet);
				if (hud.getLevel() == 21 || hud.getLevel() == 51) {
					handler.removeObject(enemyBossBullet);
				}
